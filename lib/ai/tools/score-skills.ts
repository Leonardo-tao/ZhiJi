...
const BACKEND_SKILLS_REGEX =
  /java|spring|python|node|go|rust|c\+\+|php|ruby|后端|server|backend/i;
const FRONTEND_SKILLS_REGEX =
  /react|vue|angular|javascript|typescript|html|css|前端|frontend|web/i;
const DATABASE_SKILLS_REGEX =
  /mysql|postgresql|mongodb|redis|elasticsearch|oracle|数据库|database|db/i;
const DEVOPS_SKILLS_REGEX =
  /docker|kubernetes|k8s|jenkins|git|ci\/cd|linux|运维|devops|部署/i;
const CLOUD_SKILLS_REGEX = /aws|azure|gcp|aliyun|腾讯云|华为云/i;
...
export const scoreSkills = tool({
  ...
  execute: ({ graduationYear, skills }) => {
    ...
    const hasBackendSkills = skills.some((skill) =>
      BACKEND_SKILLS_REGEX.test(skill)
    );
    const hasFrontendSkills = skills.some((skill) =>
      FRONTEND_SKILLS_REGEX.test(skill)
    );
    const hasDatabaseSkills = skills.some((skill) =>
      DATABASE_SKILLS_REGEX.test(skill)
    );
    const hasDevOpsSkills = skills.some((skill) =>
      DEVOPS_SKILLS_REGEX.test(skill)
    );
    const hasCloudSkills = skills.some((skill) =>
      CLOUD_SKILLS_REGEX.test(skill)
    );
